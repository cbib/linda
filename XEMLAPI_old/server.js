...
router.post('/register', function (req, res) {
    ...
    const keys =db._query(aql`UPSERT ${searchExpression} INSERT ${data} UPDATE {}  IN ${users} RETURN OLD `); 
...
router.get('/get_parent/:model_id', function (req, res) {
   
   var model_id=req.pathParams.model_id;
   var data={} 
...
.pathParam('model_id', joi.string().required(), 'username of the entry.')
...
router.get('/get_vertices/:user_key', function (req, res) {
    ...
    var data=[];
    data=db._query(aql`FOR v, e, s IN 1..3 ANY ${user_id} GRAPH 'global'  RETURN {e:e}`);
    res.send(data);
})
...
router.post('/saveTemplate', function (req, res) {
    ...
    var coll = db._collection("templates");

    var edge_coll='users_edge'
