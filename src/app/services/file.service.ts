...
export class FileService {
    ...
    public build_isa_model2(data, model, isa_model, return_data, model_type, filename, parent_id, parent_data) {
        console.log(parent_id)
        ...
        if (model_type==="investigation"){
            ...
            if (!already_exist){
                        ...
                        if (return_data[isa_file][index]["study_data"][isa_field]) {
                            if (return_data[isa_file][index]["study_data"][isa_field]["data"]) {
                                ...
                                if (isa_field.includes("Characteristics")) {
                                    ...
                                    return_data[isa_file][index]["study_data"][isa_field]["data"].push(data[keys[i]])
                                }
                            }
                        }
                        else{
                ...
                for (var i = 0; i < parent_keys.length; i++) {
                    if (parent_keys[i].startsWith("_") || parent_keys[i].startsWith("Definition")) {// || this.model[this.keys[i]].Level ==undefined || this.model[this.keys[i]].Level !=this.level) {
                        parent_keys.splice(i, 1);
                        i--;
                    }
                    else {
                        var is_ontology_key = this.is_ontology_key(parent_model, parent_keys[i])
                        var mapping_data = this.get_mapping_data_by_key(parent_model, parent_keys[i])
                        var isa_section = mapping_data["ISA-Tab Section (for Investigation file)"]
                        var isa_field: string = mapping_data["ISA-Tab Field"]
                        console.log("write field ", isa_field, " in section ", isa_section, " for ", isa_file)
                        if (return_data[isa_file][index]["study_data"][isa_field]) {
                            if (return_data[isa_file][index]["study_data"][isa_field]["data"]) {
                                ...
                                if (isa_field.includes("Characteristics")) {
                                    ...
                                    var term_accession_number = ""
                                    if (is_ontology_key) {
                                        term_source_ref = parent_data["v"][parent_keys[i]].split(":")[0]
                                        term_accession_number = parent_data["v"][parent_keys[i]]
                                    }
                                    let tmp_array = [parent_data["v"][parent_keys[i]], { "Term Source REF": term_source_ref }, { "Term Accession Number": term_accession_number }]
                                    return_data[isa_file][index]["study_data"][isa_field]["data"].push(tmp_array)
                                }
                                else if(isa_field ==="Factor Value[ ]"){
                                ...
                                //         let tmp_array = [factor_obj, { "Term Source REF":"", { "Term Accession Number": "" }]
                                    ...
                                    return_data[isa_file][index]["study_data"][isa_field]["data"].push(parent_data["v"][parent_keys[i]])
                                }
                            }
                        }
                        else{
                            return_data[isa_file][index]["study_data"][isa_field]={"data":[parent_data["v"][parent_keys[i]]]}
                        }
            ...
            if (!already_exist){
                ...
                return_data['Assay'][index]["assay_data"]['Sample Name']["data"].push(parent_data['v']['Observation unit ID'])
                for (var i = 0; i < keys.length; i++) {
                    if (keys[i].startsWith("_") || keys[i].startsWith("Definition")) {// || this.model[this.keys[i]].Level ==undefined || this.model[this.keys[i]].Level !=this.level) {
                        keys.splice(i, 1);
                        i--;
                    ...
                    for (var i = 0; i < return_data[isa_file].length; i++) {
                        if (return_data[isa_file][i]["filename"]===filename){
                            ...
                            for (var j = 0; j < return_data[isa_file][i]["study_data"]["Characteristics[Material Source ID]"]['data'].length; j++) {
                                ...
                                if (return_data['Study'][i]["study_data"]['Sample Name']["data"][j]===parent_data['v']['Observation unit ID']){
                                    factor_obj["factor"]=data["Experimental Factor type"]
                                    factor_obj["value"]= parent_data["v"]["Observation Unit factor value"]
