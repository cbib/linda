...
/*         this.level = 1;
                        ...
                        data => {
                            if (data["success"]) {
                                ...
                                if (this.inline==="false"){
                                    this.router.navigate(['/project_page'], { queryParams: { level: "1", parent_id: this.currentUser['_id'] , model_type: this.model_type, model_key: this.model_key , mode: "edit" } });
                                }
                                return true;
                            }
                            else {
    ...
    back(modelForm, level) {
        if (this.mode==="preprocess"){
            ...
            this.router.navigate(['/generic_form'], { queryParams: { level: parseInt(level) - 1, parent_id: this.parent_id, model_key: this.model_key, model_type: this.model_type, mode: this.mode, inline:this.inline } });
        }
